...
export const useNounsPartyCalcBidAmount = () => {
	const bidAmount = useContractCall({
		abi,
		address: config.nounsPartyAddress,
		method: "nonRevertingCalcBidAmountAfterSettle",
		args: [],
	})
	if (!bidAmount) {
		return EthersBN.from(0);
	}
	// console.info(bidAmount[1], bidAmount[2]);
	return bidAmount[1];
}
...
export const useNounsPartyWithdrawableAmount = (address: string | null | undefined) => {
	const amount = useContractCall(address && {
		abi,
		address: config.nounsPartyAddress,
