...
export const useNounsPartySettleNext = () => {
	const settleNext = useContractCall<[EthersBN]>({
		abi,
		address: config.nounsPartyAddress,
		method: "settleNextWhichDoesntRevertAsMulticallWorkaround",
		args: [],
	})
	if (!settleNext) {
		return EthersBN.from(0)
